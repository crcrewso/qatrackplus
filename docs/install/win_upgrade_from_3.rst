...
Your command prompt should now usually be prefixed with `(.venv)`, though some
shells may show `(qatrackplus)` instead.
...
For future upgrades after this renamed service is installed, run
``uv run python QATrackCherryPyService.py update`` from
``C:\deploy\qatrackplus`` to refresh the existing service definition before
restarting it.
