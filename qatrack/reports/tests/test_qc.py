...
# TestListInstanceSummaryReport tests
...
@pytest.mark.django_db
def test_summary_get_utc_site():
    """Test that get_unit_test_collection__unit__site_details works correctly."""
...
@pytest.mark.django_db
def test_summary_get_utc_freq():
    """Test that get_unit_test_collection__frequency_details works correctly."""
...
def test_summary_get_work_completed_html():
    """Test that get_work_completed works correctly for HTML format."""
    rep = qc.TestListInstanceSummaryReport()
...
@pytest.mark.django_db
@override_settings(TIME_ZONE="America/Toronto")
def test_summary_get_work_completed_plain():
    """Test that get_work_completed works correctly for non-HTML format."""
    rep = qc.TestListInstanceSummaryReport()
...
@pytest.mark.django_db
@override_settings(TIME_ZONE="America/Toronto")
def test_summary_get_pass_fail_html():
    """Test that get_pass_fail_status works correctly for HTML format."""
    rep = qc.TestListInstanceSummaryReport()
    rep.report_format = "html"
...
@pytest.mark.django_db
@override_settings(TIME_ZONE="America/Toronto")
def test_summary_get_pass_fail_plain():
    """Test that get_pass_fail_status works correctly for non-HTML format."""
    rep = qc.TestListInstanceSummaryReport()
    rep.report_format = "csv"
    tli = utils.create_test_list_instance()
    pf = rep.get_pass_fail_status(tli)
...
@pytest.mark.django_db
def test_summary_get_tlis_for_site():
    """Test that get_tlis_for_site returns correct results for a site."""
    site = USite.objects.create(name="site")
...
@pytest.mark.django_db
def test_summary_get_tlis_for_null_site():
    """Test that get_tlis_for_site returns correct results for null site."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
...
# TestListInstanceDetailsReport tests
@pytest.mark.django_db
def test_details_filter_form_valid():
    """If queryset.count() > MAX_TLIS then filter_form should get an error added."""
...
@pytest.mark.django_db
def test_details_get_unit_test_collection_details():
    """Test that get_unit_test_collection_details works correctly."""
...
# TestInstanceDetailsReport tests
@pytest.mark.django_db
def test_instance_filter_form_valid():
    """If queryset.count() > MAX_TLIS then filter_form should get an error added."""
...
def test_instance_get_queryset():
    """Test that get_queryset returns the correct model."""
    assert qc.TestInstanceDetailsReport().get_queryset().model._meta.model_name == "testinstance"


def test_instance_get_filename():
    """Test that get_filename returns the correct filename."""
    assert qc.TestInstanceDetailsReport().get_filename('pdf') == 'test-instance-details.pdf'


@pytest.mark.django_db
def test_instance_get_unit_test_info__test_details():
    """Test that get_unit_test_info__test_details works correctly."""
...
def test_instance_get_organization_details():
    """Test that get_organization_details works correctly."""
    org = qc.TestInstanceDetailsReport().get_organization_details('one_per_row')
    assert org == ('Organization', 'One Test Instance Per Row')


@pytest.mark.django_db
def test_instance_generate_html_group_by_unit_test_date():
    """Test that to_html works correctly for group_by_unit_test_date."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
    tli = utils.create_test_list_instance(unit_test_collection=utc)
...
@pytest.mark.django_db
def test_instance_generate_html_one_per_row():
    """Test that to_html works correctly for one_per_row."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
    tli = utils.create_test_list_instance(unit_test_collection=utc)
...
@pytest.mark.django_db
def test_instance_to_table_one_per_row_csv():
    """Test that to_table works correctly for one_per_row in CSV format."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
    tli = utils.create_test_list_instance(unit_test_collection=utc)
...
@pytest.mark.django_db
def test_instance_to_table_group_by_unit_test_date_csv():
    """Test that to_table works correctly for group_by_unit_test_date in CSV format."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
    tli = utils.create_test_list_instance(unit_test_collection=utc)
...
# NextDueDatesReport and DueAndOverdueQCReport tests
def test_next_due_dates_get_queryset():
    """Test that get_queryset returns the correct model."""
    assert qc.NextDueDatesReport().get_queryset().model._meta.model_name == "unittestcollection"


def test_next_due_dates_get_filename():
    """Test that get_filename returns the correct filename."""
    assert qc.NextDueDatesReport().get_filename('pdf') == 'next-due-dates-for-qc.pdf'


def test_next_due_and_overdue_filename():
    """Test that get_filename returns the correct filename for DueAndOverdueQCReport."""
    assert qc.DueAndOverdueQCReport().get_filename('pdf') == 'due-and-overdue-qc.pdf'


@pytest.mark.django_db
def test_next_due_get_unit__site_details():
    """Test that get_unit__site_details works correctly."""
    site = USite.objects.create(name="site")
...
@pytest.mark.django_db
def test_next_due_get_unit_details():
    """Test that get_unit_details works correctly."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
...
@pytest.mark.django_db
def test_next_due_generate_next_due_dates_html():
    """Test that to_html works correctly for NextDueDatesReport."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
    tli = utils.create_test_list_instance(unit_test_collection=utc)
...
@pytest.mark.django_db
def test_next_due_generate_due_and_overdue_html():
    """Test that to_html works correctly for DueAndOverdueQCReport."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
    utc = utils.create_unit_test_collection(unit=unit)
    tli = utils.create_test_list_instance(unit_test_collection=utc)
...
@pytest.mark.django_db
def test_assigned_get_unit_details():
    """Test that get_unit_details works correctly."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
...
@pytest.mark.django_db
def test_assigned_generate_summary_html():
    """Test that to_html works correctly."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
...
# AssignedQCDetailsReport tests
@pytest.mark.django_db
def test_assigned_details_filter_form_valid():
    """If queryset.count() > MAX_TLIS then filter_form should get an error added."""
...
@pytest.mark.django_db
def test_assigned_details_to_table():
    """Test that to_table works correctly."""
    site = USite.objects.create(name="site")
    unit = utils.create_unit(site=site)
