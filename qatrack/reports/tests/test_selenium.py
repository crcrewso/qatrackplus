...
class TestReportInterface(BaseQATests):
    ...
    def test_load_report(self):
        ...
        # Parse what the widget shows rather than asserting a literal. The
        # daterange picker renders using DATERANGEPICKER_DATE_FMT, which is a
        # configurable setting and does not currently agree with DATE_FORMAT -
        # so a literal here encodes today's configuration and breaks whenever
        # it changes. What the test actually cares about is that the saved
        # range came back, whatever format it is shown in.
